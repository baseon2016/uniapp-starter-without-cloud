...
import interceptors from '@/common/server-api/interceptor';


axios.defaults.timeout = configCenter.timeout;         					// 响应时间
axios.defaults.withCredentials = true; 					// 让请求头携带参数cookie
axios.defaults.baseURL = configCenter.apiUrl;			// 配置接口地址
...
new Cache(axios, {
	requestInterceptorFn: async config => {
		// 自定义请求拦截器
		//   api签名拦截器
		if (configCenter.withSignature) {
			config = await interceptors.signature(config) || config;
		}
		// 用户邀请接口拦截
		config = await interceptors.invitee(config);
		// 需要用Promise将config返回
